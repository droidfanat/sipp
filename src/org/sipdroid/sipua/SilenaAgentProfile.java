 ...
 */
public class SilenaAgentProfile extends Configure {
	...
	public SilenaAgentProfile() {
		init();
	}

	/** Costructs a new UserAgentProfile */
	public SilenaAgentProfile(String file) { // load configuration
		loadFile(file);
                android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile init");
	...
	protected void parseLine(String line) {
		...
		if (attribute.equals("from_url")) {
			from_url = par.getRemainingString().trim();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile from_url "+from_url);
			return;
		}
		if (attribute.equals("contact_url")) {
			contact_url = par.getRemainingString().trim();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile contact_url "+contact_url);
			return;
		}
		if (attribute.equals("username")) {
			username = par.getString();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile username "+username);
			return;
		}
		if (attribute.equals("realm")) {
			realm = par.getRemainingString().trim();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile realm "+realm);
			return;
		}
		if (attribute.equals("passwd")) {
			passwd = par.getRemainingString().trim();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile passwd "+passwd);
			return;
		}
		if (attribute.equals("ua_jar")) {
			ua_jar = par.getStringUnquoted();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile ua_jar "+ua_jar);
			return;
		}
		if (attribute.equals("contacts_file")) {
			contacts_file = par.getStringUnquoted();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile contacts_file "+contacts_file);
			return;
		}

		if (attribute.equals("do_register")) {
			do_register = (par.getString().toLowerCase().startsWith("y"));
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile do_register "+do_register);
			return;
		}
		if (attribute.equals("do_unregister")) {
			do_unregister = (par.getString().toLowerCase().startsWith("y"));
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile do_unregister "+do_unregister);
			return;
		}
		if (attribute.equals("do_unregister_all")) {
			do_unregister_all = (par.getString().toLowerCase().startsWith("y"));
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile do_unregister_all "+do_unregister_all);
			return;
		}
		if (attribute.equals("expires")) {
			expires = par.getInt();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile expires "+expires);
			return;
		}
		if (attribute.equals("keepalive_time")) {
			keepalive_time = par.getInt();
                             android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile keepalive_time "+keepalive_time);
			return;
		}

		if (attribute.equals("call_to")) {
			call_to = par.getRemainingString().trim();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile call_to "+call_to);
			return;
		}
		if (attribute.equals("accept_time")) {
			accept_time = par.getInt();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile accept_time "+accept_time);
			return;
		}
		if (attribute.equals("hangup_time")) {
			hangup_time = par.getInt();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile hangup_time: "+hangup_time);
			return;
		}
		if (attribute.equals("transfer_time")) {
			transfer_time = par.getInt();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile transfer_time "+transfer_time);
			return;
		}
		if (attribute.equals("re_invite_time")) {
			re_invite_time = par.getInt();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile re_invite_time "+re_invite_time);
			return;
		}
		if (attribute.equals("redirect_to")) {
			redirect_to = par.getRemainingString().trim();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile redirect_to "+redirect_to);
			return;
		}
		if (attribute.equals("transfer_to")) {
			transfer_to = par.getRemainingString().trim();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile transfer_to "+transfer_to);
			return;
		}
		if (attribute.equals("no_offer")) {
			no_offer = (par.getString().toLowerCase().startsWith("y"));
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile no_offer "+no_offer);
			return;
		}
		if (attribute.equals("no_prompt")) {
			no_prompt = (par.getString().toLowerCase().startsWith("y"));
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile no_prompt" +no_prompt);
			return;
		}

		if (attribute.equals("audio")) {
			audio = (par.getString().toLowerCase().startsWith("y"));
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile audio "+audio);
			return;
		}
		if (attribute.equals("video")) {
			video = (par.getString().toLowerCase().startsWith("y"));
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile video "+video);
			return;
		}
		if (attribute.equals("recv_only")) {
			recv_only = (par.getString().toLowerCase().startsWith("y"));
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile recv_only "+recv_only);
			return;
		}
		if (attribute.equals("send_only")) {
			send_only = (par.getString().toLowerCase().startsWith("y"));
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile send_only "+send_only);
			return;
		}
		if (attribute.equals("send_tone")) {
			send_tone = (par.getString().toLowerCase().startsWith("y"));
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile send_tone "+send_tone);
			return;
		}
		if (attribute.equals("send_file")) {
			send_file = par.getRemainingString().trim();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile send_file "+send_file);
			return;
		}
		if (attribute.equals("recv_file")) {
			recv_file = par.getRemainingString().trim();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile recv_file"+recv_file);
			return;
		}

		if (attribute.equals("audio_port")) {
			audio_port = par.getInt();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile audio_port "+audio_port);
			return;
		}
		if (attribute.equals("audio_sample_rate")) {
			audio_sample_rate = par.getInt();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile audio_sample_rate "+audio_sample_rate );
			return;
		}
		if (attribute.equals("audio_sample_size")) {
			audio_sample_size = par.getInt();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile audio_sample_size "+audio_sample_size);
			return;
		}
		if (attribute.equals("audio_frame_size")) {
			audio_frame_size = par.getInt();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile audio_frame_size "+audio_frame_size);
			return;
		}
		if (attribute.equals("video_port")) {
			video_port = par.getInt();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile video_port "+video_port);
			return;
		}
		if (attribute.equals("video_avp")) {
			video_avp = par.getInt();
                            android.util.Log.d(MainConstant.LOG_TAG, "UserAgent Profile video_avp "+video_avp);
			return;
		}
